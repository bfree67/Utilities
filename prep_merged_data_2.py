...
### drop index column if present and remove rows with NaNs
...
df_nn = df_nn.reset_index(drop=True)   ### reset index
...
df_maxmin = pd.DataFrame(scal_max_min)
...
df_data = pd.concat((df_cyclic, df_scaled), axis = 1)
...
df_data.to_excel(writer) # save to Excel
...
print('\nPreparing data tensor for look ahead = ' + str(look_ahead))
