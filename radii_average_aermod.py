...
writer = pd.ExcelWriter('aermod_ave_results.xlsx')
...
for scenario in folder:
    ...
    pathname = 'C:/Users/Brian\\Documents/_Active/BOEM/Flares/AERMOD/' + scenario + '/'+ scenario +'.AD/'
    os.chdir(pathname)
    ...
    files = glob.glob("*.PLT")  # find plot files
    files = files[0:3]  # remove PE00GALL.PLT file
    
    for grid_file in files:   #read all the files in the directory with .PLT extension
    
        print(grid_file)
        df_new= read_csv(grid_file, engine='python', skiprows=7, delim_whitespace=True)
        new_cols = list(df_new)
        df_grid = df_new.iloc[:,0:3]
        df_grid.columns = ['x','y','conc']
        df_grid = df_grid.apply(lambda x: pd.to_numeric(x, errors = 'coerce')).dropna() #remove strings
        df_grid = df_grid.reset_index(drop = True)

        
        # add column with distance from source (source at 0,0)
        df_grid['distance'] = (((df_grid['x']-x0)**2 + (df_grid['y']-y0)**2)**.5)/1000
